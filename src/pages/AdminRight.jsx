  ...
  document.getElementById("homeright").style.display = "none"
    ...
    return ( 
        ...
        <div className="chat-header" id='headerRight'>
            ...
            <div className="message-chat-name">{data.user.uid && users[users.findIndex(user => user.uid === data.user.uid)].firstName}</div>
            </div>
        ...
        <div className="chats" id='chatBox' onClick={hideContact}>
        ...
        {messages?.sort((a,b)=>a[1].date - b[1].date).map((chat) => (
          <div className={currentUser.uid === chat[1].senderId? "owner": "other"}
           key={chat[0]}>
            {chat[1].img? <a href={chat[1].img}><img className='chat-image' src={chat[1].img}  download/></a> : <span>{chat[1].text}</span>}
           </div>
